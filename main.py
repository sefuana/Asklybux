...
async def handle_task_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Force remove reply keyboard first
    ...
    await update.message.reply_text(
        msg, parse_mode=ParseMode.MARKDOWN, reply_markup=cancel_kb
    )

async def handle_task_email(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    email = update.message.text.strip()

    # Ignore leftover keyboard button presses from previous state
    if email in ("▶️ Start", "Cancel ❌", "🔙 Back", "Account Registered ✅"):
        await update.message.reply_text(
            "📧 Please *type* your email or phone number and send it.",
...
# ──────────────────────────────────────────────
#  /wd — withdrawal requests (one-by-one panel)
# ──────────────────────────────────────────────
async def cmd_wd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["wd_list"]  = pending
    ctx.user_data["wd_index"] = 0

    await send_wd_card(update, ctx, edit=False)


async def send_wd_card(update_or_query, ctx: ContextTypes.DEFAULT_TYPE, edit=False):
    pending = ctx.user_data.get("wd_list", [])
    idx     = ctx.user_data.get("wd_index", 0)

    if not pending:
        text = "📭 No more pending withdrawal requests."
        if edit:
            await update_or_query.edit_message_text(text)
        else:
            await update_or_query.message.reply_text(text)
    ...
    t       = v.get("requested_at", "N/A")

    text = (
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"💸  Withdrawal {idx+1}/{len(pending)}\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"👤  Name:        `{name}`\n"
        f"🆔  ID:            `{uid}`\n"
        f"📛  Username:  `{uname}`\n"
    ...
    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("❌ Cancel", callback_data=f"wd_cancel:{k}:{uid}"),
            InlineKeyboardButton("✅ Approve", callback_data=f"wd_approve:{k}:{uid}:{amount}"),
        ],
        [
            InlineKeyboardButton("⬅️ Previous", callback_data="wd_prev"),
            InlineKeyboardButton("➡️ Next",     callback_data="wd_next"),
        ],
    ])
...
async def callback_wd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    data  = query.data

    if data == "wd_prev":
        ctx.user_data["wd_index"] = max(0, ctx.user_data.get("wd_index", 0) - 1)
        await send_wd_card(query, ctx, edit=True)
        return

    if data == "wd_next":
        pending = ctx.user_data.get("wd_list", [])
        ctx.user_data["wd_index"] = min(len(pending)-1, ctx.user_data.get("wd_index", 0) + 1)
        await send_wd_card(query, ctx, edit=True)
        return

    if data.startswith("wd_cancel:"):
        _, wd_key, uid_str = data.split(":")
        uid = int(uid_str)

        fb_update(f"withdrawals/{wd_key}", {"status": "cancelled"})
        ...
        try:
            await ctx.bot.send_message(
                ...
                "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                "😔 Your withdrawal request has been *cancelled* by admin.\n\n"
        ...
        # Remove from local list
        pending = ctx.user_data.get("wd_list", [])
        ctx.user_data["wd_list"]  = [(k, v) for k, v in pending if k != wd_key]
        ctx.user_data["wd_index"] = max(0, ctx.user_data.get("wd_index", 0))

        await query.edit_message_text(
            f"✅ Withdrawal `{wd_key}` cancelled. User notified.",
            parse_mode=ParseMode.MARKDOWN,
        )
        return
    ...
    if data.startswith("wd_approve:"):
        ...
        fb_update(f"withdrawals/{wd_key}", {"status": "approved"})

        # Get address for notification
        wd_data = fb_get(f"withdrawals/{wd_key}") or {}
        address = wd_data.get("address", "N/A")
        ...
        try:
            await ctx.bot.send_message(
                ...
                f"📋  Address:   `{address}`\n"
                f"💵  Amount:    `${amount:.4f}`\n\n"
                "⏳ Funds should arrive within a few minutes.\n"
        ...
        await query.edit_message_text(
            f"✅ Withdrawal `{wd_key}` approved!\n"
            f"💵 `${amount:.4f}` sent to user `{uid}`.\n"
            f"📋 Address: `{address}`",
            parse_mode=ParseMode.MARKDOWN,
        )
...
# ──────────────────────────────────────────────
#  Task Cancel (inline button callback)
# ──────────────────────────────────────────────
async def callback_task_cancel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ctx.user_data[STATE_KEY] = STATES["IDLE"]
    ...
    await ctx.bot.send_message(
        ...
        "❌ Task cancelled. Back to main menu.",
        reply_markup=MAIN_MENU,
    )
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(callback_wd, pattern="^wd_"))
    app.add_handler(CallbackQueryHandler(callback_task_cancel, pattern="^task_cancel$"))
